<div>
    <form wire:submit.prevent="save" method="post" novalidate>
        ...
        <div class="w-100">
            ...
            <div class="mb-3">
                <x-form-label id="perusahaans" label='Perusahaan' />
                <div class="row ms-1">
                    @foreach ($perusahaans as $perusahaan)
                    <div class="form-check col-sm-4">
                        <input class="form-check-input" type="checkbox" value="{{ $perusahaan->id }}"
                            wire:model.defer="perusahaan_ids.{{ $perusahaan->id }}"
                            id="flexCheckPerusahaan{{ $loop->index }}">
                        <label class="form-check-label" for="flexCheckPerusahaan{{ $loop->index }}">
                            {{ $perusahaan->name }}
                        </label>
                    </div>
                    @endforeach
                </div>
                <small class="text-muted d-block mt-1">Pilih perusahaan yang akan menggunakan absensi ini.</small>
                <x-form-error key="perusahaan_ids" />
