<div>
    <form wire:submit.prevent="saveEmployees" method="post"  enctype="multipart/form-data" novalidate >
        ...
        <div class="mb-3">
            <div class="row g-3">
                <div class="col-md-6">
                    <x-form-label id="name{{ $i }}" label='Nama Karyawaan' />
                    <x-form-input id="name{{ $i }}" name="name{{ $i }}" wire:model.defer="employees.{{ $i }}.name" />
                    <x-form-error key="employees.{{ $i }}.name" />
                </div>
                <div class="col-md-6">
                    <x-form-label id="username{{ $i }}" label='Username Karyawaan ' />
                    <x-form-input id="username{{ $i }}" name="username{{ $i }}" wire:model.defer="employees.{{ $i }}.username" />
                    <x-form-error key="employees.{{ $i }}.username" />
                </div>
                <div class="col-md-6">
                    <x-form-label id="email{{ $i }}" label='Email Karyawaan' />
                ...
                <div class="col-md-6">
                    <x-form-label id="phone{{ $i }}" label='No. Telp' />
                ...
                <div class="col-md-6">
                    <x-form-label id="password{{ $i }}"
                        label='Password Karyawaan (default: "123" jika tidak diisi)' required="false" />
                ...
                <div class="col-md-6">
                    <x-form-label id="perusahaan_id{{ $i }}" label='Perusahaan' />
                    <select class="form-select" aria-label="Default select example" name="perusahaan_id"
                        wire:model.defer="employees.{{ $i }}.perusahaan_id">
                        <option selected disabled>-- Pilih Perusahaan --</option>
                        @foreach ($perusahaans as $perusahaan)
                        <option value="{{ $perusahaan->id }}">{{ ucfirst($perusahaan->name) }}</option>
                        @endforeach
                    </select>
                    <x-form-error key="employees.{{ $i }}.perusahaan_id" />
                </div>
                <div class="col-md-6">
                    <x-form-label id="position_id{{ $i }}" label='Jabatan / Posisi Karyawaan' />
                ...
                <div class="col-md-6">
                    <x-form-label id="role_id{{ $i }}" label='Role' />
